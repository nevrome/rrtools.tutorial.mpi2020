...
Open RStudio or log into this [RStudio Cloud session](https://rstudio.cloud/spaces/49056/join?access_code=TO1DZn5WUKxm1Bo1xn7hjm7GYwWxxOs9vU4yyvI9).
...
[rrtools](https://github.com/benmarwick/rrtools) is a **mighty wizard** package witten by Ben Marwick and colleagues that facilitates some of the steps of Research Compendium creation and maintenance. It has a **lot of dependencies** and it is **strongly opinionated**.
...
take a look at the `./analysis/paper/paper.Rmd` file, render it with the `Knit` button of RStudio and inspect the resulting `.docx` file
...
create a file `names.txt` in `./analysis/raw_data` that contains the first names of some of the people around you
...
edit `./analysis/paper/paper.Rmd` to include our analysis in an R code chunk

```{r}
names_vector <- readLines(here::here("analysis/data/raw_data/names.txt"))

plot_name_dist_matrix(names_vector)
```
...
Code and data (!) in a Research Repository should come with license declaration that declare the copyright holder and what can and can not be done legally with your intellectual property.
